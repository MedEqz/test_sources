...
const leftArrowSequence = (currentScore: number, i: number, split: string) => {
  ...
  currentScore += 1;
  console.log("left arrow score of < :", currentScore);
  ...
  if (!simpleArrow && split[i + 1] === DOUBLE) {
    ...
    while (split[i + 1] === DOUBLE) {
      currentScore += 1;
      i += 1;
      console.log("left double arrow score of = :", currentScore);
    }
  }
  ...
  if (!simpleArrow && !doubleArrow && split[i + 1] === HALF) {
    ...
    while (split[i + 1] === HALF) {
      currentScore += 1;
      i += 1;
      console.log("left half arrow score of ~ :", currentScore);
    }
  }
  ...
  if (split[i + 1] === RIGHT) {
    ...
    console.log("left arrow bi direction : sequence score is reset to 0");
  }
...
const rightArrowSequence = (currentScore: number, i: number, split: string) => {
  ...
  currentScore += 1;
  console.log("right arrow score of > :", currentScore);
...
const genericSequences = (
  ...
  currentScore += 1;
  console.log(`1 genericSequences for ${symbol} : ${currentScore}`);
  ...
  while (sequence) {
    if (!rightArrow && split[i + 1] === symbol) {
      currentScore += 1;
      i += 1;
      console.log(`2 genericSequences for ${symbol} : ${currentScore}`);
    }
    ...
    if (split[i + 1] === RIGHT) {
      ...
      currentScore += 1;
      i += 1;
      console.log(`3 genericSequences for ${RIGHT} : ${currentScore}`);
      ...
      if (split[i + 1] === RIGHT) {
        ...
        currentExponant += 1;
        i += 1;
        console.log(`4 genericSequences for ${RIGHT} : ${currentScore}`);
        ...
        while (powerRightArrow) {
          if (split[i + 1] === RIGHT) {
            ...
            currentScore += 1;
            i += 1;
            console.log(`5 right arrow score of ${RIGHT} : ${currentScore}`);
          } else {
    ...
    if (!rightArrow && split[i + 1] !== symbol) {
      sequence = false;
      currentScore = 0;
      console.log("genericSequences is null");
    }
    ...
    if (rightArrow && split[i + 1] !== RIGHT) {
      ...
      console.log("genericSequences ended");
    }
  }
  return { currentScore, i };
};
 ...
 * @returns inputScore
 */
const arrowCount = (inputText: string): number => {
  ...
  for (const split of splitedArrowInput) {
    console.log("======== splited fragment ======== :", split);
